"""
This CRF implementation is taken from AllenNLP
https://github.com/allenai/allennlp.

We don't directly include AllenNLP as a dependency of this project because of
the package force use to install unused huge packages such as Spacy.
reference: https://github.com/allenai/allennlp/issues/1908
"""
# TODO: remove this code and call AllenNLP directly when the issue 1908 solved
...
def viterbi_decode(tag_sequence: torch.Tensor,
    ...
    for timestep in range(1, sequence_length):
        ...
        summed_potentials = path_scores[timestep - 1].unsqueeze(-1) + \
                            transition_matrix
...
class ConditionalRandomField(torch.nn.Module):
    ...
    def _input_likelihood(self, logits: torch.Tensor,
        ...
        """
        Use forward algorithm to compute the (batch_size,)
        denominator term for the log-likelihood, which is the
    ...
    def neg_log_likelihood(self,
        ...
        """
        Computes the negative log likelihood of the target tags (y)
        given input emission logits
        """
        ...
        # log score(x,y)
        input_score = self._joint_likelihood(emission_logits, tags, mask)

        # - log p(y|x) = - log (score(x,y) / Z(x))
        #              = - [ log score(x,y) - log Z(x) )
        return -1 * torch.mean(input_score - normalize_term)
    ...
    def forward(self,
        ...
        Args:
            emission_logits: (batch_size, seq_len, tag_size)
            mask: (batch_size, seq_len)
                a masking tensor with 0 at padding position, otherwise 1

        Returns: (batch_size)
            A list of best path and score from Viterbi algorithm
        """
        _, max_seq_length, num_tags = emission_logits.size()

        # Get the tensors out of the variables
        emission_logits, mask = emission_logits.data, mask.data
        ...
        # iterate sequence by sequence
        for prediction, prediction_mask in zip(emission_logits, mask):
